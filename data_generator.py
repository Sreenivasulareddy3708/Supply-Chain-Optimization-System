...
# Write to CSV in chunks to avoid memory overflow
chunk_size = 100000
chunk_buffer = []
demand_csv_path = os.path.join(DATA_OUTPUT_DIR, 'demand.csv')
...
for _, product_row in dim_products_df.iterrows():
    ...
    for _, location_row in dim_locations_df.iterrows():
        ...
        for i, current_date in enumerate(date_range):
            ...
            # Write chunk when buffer reaches size limit
            if len(chunk_buffer) >= chunk_size:
                chunk_df = pd.DataFrame(chunk_buffer, columns=['date', 'product_id', 'location_id', 'units_sold'])
                mode = 'w' if not os.path.exists(demand_csv_path) else 'a'
                header = not os.path.exists(demand_csv_path)
                chunk_df.to_csv(demand_csv_path, mode=mode, header=header, index=False)
                chunk_buffer = []
...
# Count rows in demand.csv
demand_row_count = sum(1 for line in open(demand_csv_path)) - 1  # Subtract header
...
print(f"  - demand.csv ({demand_row_count} rows)")
...
# Step 2: Train XGBoost on Prophet residuals
# residuals = actual_demand - prophet_forecast['yhat']
# xgb_model = XGBRegressor()
# xgb_model.fit(features, residuals)

# Step 3: Adaptive weighting
# prophet_weight = calculate_weight(prophet_errors)
# xgb_weight = 1 - prophet_weight
# final_forecast = (prophet_weight * prophet_pred) + (xgb_weight * xgb_pred)

            ...
            if train_data.empty or len(train_data) < 2:
                ...
                if not validation_data.empty: # Only predict if there's actual validation data
                    ...
                    if future_validation.empty or 'ds' not in future_validation.columns:
                        ...
                        # Handle duplicate dates by grouping
                        validation_data_grouped = validation_data.groupby('ds')['y'].mean().reset_index()
                        actual_validation_aligned = validation_data_grouped.set_index('ds').reindex(forecast_validation_df['ds'])['y'].values
